...
def main():
    ...
    print("\n" + "5-)makeblastdb done. Running blast with the assembled hifiasm contigs" + "\n")
    ...
    for contig_id in contigs_ids:
        ...
        annotate_mito = "mitofinder -j {}.annotation -a {}.mitogenome.fa -r {} -o {}".format(contig_id, contig_id, args.g, args.o)
        ...
        if start == None:
            raise Exception('tRNA-Phe is not present in file ', mitogenome_gb)
        new_gb = None
        if strand == -1:
            genome_rc = contig_id + ".RC_mitogenome.fa"
            rc = os.path.join(os.path.dirname(genome), genome_rc)
            rotation.make_rc(genome, rc)
            new_gb = rotation.annotate(os.path.dirname(genome), os.path.abspath(genome_rc), os.path.abspath(args.g), contig_id, args.o)
    ...
    # then run MAFFT alignment between the rotated contigs using the multifasta as input
    alignContigs.mafft_align(multifasta_file=concat_fasta, threads=args.t)
