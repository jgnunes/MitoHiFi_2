...
def get_circo_mito(contig_id, circular_size, circular_offset):
    ...
    """
    logger.debug("Defining cut_coords function...\n")
    def cut_coords(contig_fasta, circularization_position, fasta_out):
        logger.debug(f"cut_coords function called!\ncontig_fasta: {contig_fasta}; circularization_position: {circularization_position}; fasta_out: {fasta_out}") 
    ...
    logger.debug(f"circularization_history: {circularization_history}")
...
def process_contig(threads_per_contig, circular_size, circular_offset, contigs, max_contig_size, rel_gbk, gen_code, contig_id): 
    ...
    logger.debug(f"contigs: {contigs} | args.circular_size: {circular_size}") 
    ...
    logger.debug("Running mitofinder with maximum contig size of {} bp".format(max_contig_size))
    ...
    if not os.path.isfile(mitogenome_gb):
        logger.warning("Contig "+ contig_id + " does not have an annotation file, check MitoFinder's output")
        return
    start, strand = rotation.get_phe_pos(mitogenome_gb)
    genome = contig_id + ".mitogenome.fa"
    if start == None:
        logger.warning('tRNA-Phe is not present in file ' + mitogenome_gb + '... Skipping contig ' + contig_id + '\n')
        return
    new_gb = None
    if strand == -1:
    ...
    logger.info(' '.join(['Rotated to tRNA-Phe genome is at', os.path.join(os.path.dirname(genome), contig_id + '.mitogenome.rotated.fa')]))
    if new_gb:
        logger.debug("new_gb: ")
        logger.debug(new_gb)
        logger.debug('Mitogenome annotation is at ', new_gb)
...
def main():
    ...
    parser.add_argument("-d", help="-d: print log in DEBUG mode (extra information gets printed)", default="False", choices=["True", "False"])
    ...
    logger.info("MitoHifi v2.2\n")
    logger.info(f"Started at: {today}")
    ...
    logger.info(f"Length of related mitogenome is: {rel_mito_len} bp")
    logger.info(f"Number of genes on related mitogenome: {rel_mito_num_genes}")
    ...
    if args.r:
        ...
        logger.info("Fixing potentially conflicting FASTA headers...\n")
    ...
    #with concurrent.futures.ProcessPoolExecutor() as executor:
        #futures = [executor.submit(partial_process_contig, contig_id) for contig_id in contigs_ids]
        #wait(futures)
    ...
    logger.debug(f"circularization_history_files: {circularization_history_files}") 
    ...
    logger.debug(f"contig_shifts after parallel processing: {contig_shifts}") 
    ...
    logger.debug("List of contigs that will be aligned: " + str(contigs_files) + "\n")
    ...
    logger.info("Representative contig is {} that belongs to {}. This contig will be our final mitogenome. See all contigs and clusters in cdhit.out.clstr".format(repr_contig_id, repr_contig_cluster))
    ...
    logger.debug(f"contigs_stats_files: {contigs_stats_files}")
