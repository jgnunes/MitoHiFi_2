...
def main():
    ...
    if args.r:
        ...
        get_p = "{} gbk.HiFiMapped.bam.filtered.assembled.p_ctg.gfa >  gbk.HiFiMapped.bam.filtered.assembled.p_ctg.fa".format(gfa2fa_script)
        get_a = "{} gbk.HiFiMapped.bam.filtered.assembled.a_ctg.gfa >  gbk.HiFiMapped.bam.filtered.assembled.a_ctg.fa".format(gfa2fa_script)
    ...
    # creates a file that will save contigs stats
    ...
    for contig_id in contigs_ids:
        ...
        # check frameshifts in genes from contig and append findings to 
        # `contigs_stats.tsv` file
        frameshifts = findFrameShits.find_frameshifts(mitogenome_gb)
        contig_shifts[contig_id] = frameshifts
        ...
        with open("contigs_stats.tsv", "a+") as f:
            if not frameshifts:
                ...
                all_frameshifts = str(frameshifts)
            elif len(frameshifts)>1:
                all_frameshifts = ";".join(frameshifts)
            mitogenome_location = os.path.join(contig_dir, mitogenome_gb)
            f.write("\t".join([contig_id, all_frameshifts, mitogenome_location + "\n"]))
    
    print("Frameshifts found:", contig_shifts) 
    logging.info("Frameshifts detected for each contig:", contig_shifts)
