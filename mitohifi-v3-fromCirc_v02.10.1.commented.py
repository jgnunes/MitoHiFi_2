...
#    alignContigs.mafft_align(multifasta_file=concat_fasta, threads=args.t, clustal_format=True)
    ...
    # check if representative contig was reverse complemented (RC)
    repr_contig_gb = os.path.join(repr_contig_id + ".annotation", repr_contig_id + ".annotation_MitoFinder_mitfi_Final_Results", repr_contig_id + ".annotation_mtDNA_contig.gb") 
    repr_contig_gb_rc = os.path.join(repr_contig_id + "_RC.annotation", repr_contig_id + "_RC.annotation_MitoFinder_mitfi_Final_Results", repr_contig_id + "_RC.annotation_mtDNA_contig.gb") 
    
    if os.path.isfile(repr_contig_gb_rc):
        repr_contig_gb_final = repr_contig_gb 
    else:
        repr_contig_gb_final = repr_contig_gb
    
    print("Representative contig: ", repr_contig_gb_final) # for debugging, delete later
    # create a (genbank) copy of the representative contig called "final_mitogenome.gb"
    record=SeqIO.read(repr_contig_gb_final, "genbank")
    record.id = "final_mitogenome"
    with open("final_mitogenome.gb", "w") as f:  
        f.write(record.format('genbank'))        
    # create a (fasta) copy of the representative contig called "final_mitogenome.fasta" 
    record=SeqIO.read(repr_contig_gb_final.replace(".gb", ".fasta"), "fasta")
    record.id = "final_mitogenome"
    with open("final_mitogenome.fasta", "w") as f:  
        f.write(record.format('fasta'))        
