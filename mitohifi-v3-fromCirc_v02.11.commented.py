...
#    subprocess.run(repr_contig_get_gb, shell=False)
    ...
    if (not os.path.isfile(final_gbk)) or (not os.path.isfile(final_fasta)):
        raise Exception("Final mitogenome has not been created, finishing pipeline with an error. Please check log files for more details.")
    ...
    # creating directories for each contig and movg their content there (to clean up working directory)
