...
def annotate_mito(contig_id, max_contig_size, related_gbk, gen_code, threads):
    #mitofinder = "mitofinder --max-contig-size {} -j {}.annotation -a {}.mitogenome.fa -r {} -o {} -p {}".format(max_contig_size, contig_id, contig_id, related_gbk, gen_code, threads)
    mitofinder = ["/software/team311/jf18/MitoFinder/mitofinder", "--max-contig-size", str(max_contig_size), 
            "-j", contig_id + ".annotation", "-a", contig_id + ".mitogenome.fa", "-r", related_gbk, "-o", 
            gen_code, "-p", str(threads)]
    return subprocess.run(mitofinder, shell=False)
#def annotate_mito(contig_id, max_contig_size="80000", related_gbk="MK903032.1.gb", gen_code="5", threads="1"):
   #mitofinder = ["/software/team311/jf18/MitoFinder/mitofinder", "--max-contig-size", max_contig_size, 
            #"-j", contig_id+".annotation", "-a", contig_id+".mitogenome.fa", "-r", related_gbk, "-o", 
            #gen_code, "-p", threads]
    #return subprocess.run(mitofinder, shell=False)

def rotate_mito(contig_id, related_gbk, gen_code, max_contig_size, threads_per_contig):
    ...
    new_gb = None
    if strand == -1:
        genome_rc = contig_id + ".RC_mitogenome.fa"
        rc = os.path.join(os.path.dirname(genome), genome_rc)
        rotation.make_rc(genome, rc)
        new_gb = rotation.annotate(os.path.dirname(genome), os.path.abspath(genome_rc), os.path.abspath(related_gbk), contig_id, gen_code, max_contig_size, threads_per_contig)
...
def main():
    ...
    # annotates the mitogenomes using mitofinder
    ...
    ## maximum number of threads dedicated for each contig 
    if len(contigs_ids) > args.t:
        threads_per_contig = 1 
    else:
        threads_per_contig = args.t//len(contigs_ids)

    annotate_mito_partial = partial(annotate_mito, max_contig_size=max_contig_size, related_gbk=args.g, gen_code=args.o, threads=threads_per_contig)
    pool = multiprocessing.Pool(processes=args.t)
    
    results = pool.map(annotate_mito_partial,contigs_ids)
    print(results)
    
    print("Rotating the potential mitogenomes...")

    rotate_mito_partial = partial(rotate_mito, max_contig_size=max_contig_size, related_gbk=args.g, gen_code=args.o, threads_per_contig=threads_per_contig)
    pool = multiprocessing.Pool(processes=args.t)

    pool.map(rotate_mito_partial, contigs_ids)
